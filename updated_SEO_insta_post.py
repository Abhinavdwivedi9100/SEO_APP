...
def content_summarizer(content):
    ...
    return content, final_response[0]
...
def select_keywords(keywords, condition):
    return [keyword for keyword in keywords if condition(keyword)]

def post_reply(reddit, submission_id, reply_text):
    try:
        submission = reddit.submission(id=submission_id)
        submission.reply(reply_text)
        st.write(f"Replied to submission with ID: {submission_id}")
    except Exception as e:
        st.write(f"Error posting reply: {e}")

# Define the break_keywords function
def break_keywords(keywords):
    return keywords

#Define the search_questions function
def search_questions(reddit, keyword_pieces, subreddit_name):
    query = ' OR '.join(keyword_pieces)
    try:
        if subreddit_name:
            results = list(reddit.subreddit(subreddit_name).search(query, sort='relevance', time_filter='all', limit=5))
        else:
            results = list(reddit.subreddit("all").search(query, sort='relevance', time_filter='all', limit=5))
...
def generate_answer(openai_client, question_title, context):
    try:
        ...
        messages = [
            ...
            {"role": "user", "content": f"Here's the topic or question. Provide a relevant and brief answer in about 250 words to the following:\n\n{question_title}"}
        ]
...
if 'summarized_content' in st.session_state:
    ...
    try:
        ...
        if keywords is None:
            raise ValueError("Keyword generation failed")
    ...
    try:
        selected_from_dropdown = st.multiselect("Select keywords to search", options=keywords, default=[])
        new_keywords = st_tags(label='Add new keywords:', text='Press enter to add more', value=[], suggestions=keywords)
    ...
    if st.button("Search and Post"):
        if client_id and client_secret and reddit_password and user_agent and reddit_username and st.session_state['selected_keywords'] and st.session_state['selected_subreddits']:
            try:
                ...
                if reddit is None:
                    raise ValueError("Reddit initialization failed")
